...
export class BoardUpdatePage {
  ...
  noticeListInput = element(by.id('field_noticeList'));
  ...
  async setNoticeListInput(noticeList: string): Promise<void> {
    await this.noticeListInput.sendKeys(noticeList);
  }

  async getNoticeListInput(): Promise<string> {
    return await this.noticeListInput.getAttribute('value');
  }
